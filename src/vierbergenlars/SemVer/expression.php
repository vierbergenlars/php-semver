...
class expression {
	...
	static function standarize($version,$padZero=false) {
		...
		if(!preg_match($expression,$version,$matches)) throw new SemVerException('Invalid version string given');
	...
	static protected function rangesToComparators($range) {
		...
		if(!preg_match($expression,$range,$matches)) throw new SemVerException('Invalid range given');
